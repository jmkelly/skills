...
3. Otherwise, investigate the offenders in each failing queue (`crap-queue.md`,
   `metrics-queue.md`, `stryker-queue.md`), worst first: read the source around each
   `file:line`, and for **each** offender record:
   - Method, file:line, metric (CRAP with complexity + coverage components, or MI/cc, or survived mutant)
   - Diagnosis: what makes it complex (nesting depth, branch count, mixed responsibilities,
     duplicated logic, untested path, test gap that lets a mutant survive)
...
splits, swallowed exceptions or removed validation); catching those is your job.